...
export function PageLoadingState({ label = 'Loading page', className }: LoadingStateProps) {
  return (
    <div
      className={cn('mx-auto w-full max-w-[var(--editable-container)] px-5 py-16 sm:px-8 lg:px-12', className)}
      aria-live="polite"
      aria-busy="true"
    >
      <p className="editable-mono text-[11px] uppercase tracking-[0.16em] text-[var(--slot4-accent)]">{label}</p>
      <PulseBlock className="mt-6 h-14 w-3/4 max-w-3xl" />
      <PulseBlock className="mt-5 h-5 w-2/3 max-w-2xl" />
      <div className="mt-12 grid gap-6 md:grid-cols-3">
        {[0, 1, 2].map((item) => (
          <div key={item} className="rounded-3xl border border-[var(--editable-border)] bg-white/[0.02] p-6">
  ...
  return (
    <div className={cn('grid gap-6 sm:grid-cols-2 lg:grid-cols-3', className)} aria-live="polite" aria-busy="true">
      {Array.from({ length: count }).map((_, index) => (
        <div key={index} className="rounded-3xl border border-[var(--editable-border)] bg-white/[0.02] p-5">
          <PulseBlock className="aspect-[16/10] w-full" />
...
export function DetailLoadingState({ label = 'Loading', className }: LoadingStateProps) {
  return (
    <div
      className={cn('mx-auto grid w-full max-w-[var(--editable-container)] gap-10 px-5 py-16 sm:px-8 lg:grid-cols-[0.8fr_1.2fr] lg:px-12', className)}
      aria-live="polite"
      aria-busy="true"
    >
      <PulseBlock className="h-96 w-full rounded-3xl" />
      <div>
        <p className="editable-mono text-[11px] uppercase tracking-[0.16em] text-[var(--slot4-accent)]">{label}</p>
